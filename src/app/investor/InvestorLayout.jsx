...
const navLinks = [
  ...
  { name: "Deals", href: "/investor/deals" },
  { name: "CMS", href: "/investor/cms" },
  { name: "Users", href: "/investor/users" },
];
    ...
    <div className="min-h-screen ">
      ...
      <nav className="fixed top-0 left-0 right-0 h-18 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-10 z-30 box-shadow ">
        <div className="flex items-center gap-2">
          ...
          <Link href="/investor" className="text-xl sm:text-2xl font-bold primary-color focus:outline-none">
            Equigini
          </Link>
        </div>
        <div className="flex items-center gap-2">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className={`hidden md:inline-block px-3 py-2 rounded hover:bg-purple-50 link-text transition-colors ${
                pathname === link.href
